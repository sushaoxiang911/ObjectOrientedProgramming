...
private:
    ...
    double fuel;                // the fuel on this island
    double production_rate;     // the fuel production rate of this island

	// forbid  copy/move, construction/assignment
    Island(const Island& rhs);
    Island(Island&& rhs);
    Island& operator= (const Island& rhs);
    Island& operator= (Island&& rhs);
};
